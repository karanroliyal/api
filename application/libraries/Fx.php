...
class Fx
{
    ...
    public function __construct()
    {
        ...
        if (!isset($_POST['table_name']) || empty($_POST['table_name'])) {
            echo  $this->api_response(400, '', 'Table name is required');
            die;
        }
        if (!isset($_POST['action']) || empty($_POST['action'])) {
            echo   $this->api_response(400, '', 'Action is required');
            die;
        }
    ...
    public function user_log_creator($action, $data, $action_table, $action_id, $menu_name)
    {
        ...
        if ($action == 'insert') {
            ...
            $message = "$user_name (id : $user_id) has get data for edit of id ($action_id) from $action_table table";
        } else if ($action == 'get list') {
            $message = "$user_name (id : $user_id) has get list  ($action_id) from $action_table table";
        } else if ($action == 'get rights') {
            $message = "$user_name (id : $user_id) has get rights for id ( $action_id )";
        } else if ($action == 'set rights') {
            $message = "$user_name (id : $user_id) has set rights for id ( $action_id )";
        }
